...
この画像に写っている商品を詳しく分析し、特定してください。
画像内のテキスト、ロゴ、形状、特徴から推論を行ってください。

以下の情報を日本語で出力してください：
1. 商品名（メーカー・ブランド名、型番があればそれも含める）
2. 商品の短い説明
3. 検索に使用するためのキーワード（メルカリ等の市場検索に最適な単語を3-5個、スペース区切りで）
4. 定価（メーカー希望小売価格や発売時の価格。知識ベースから推論し、具体的な金額を記載してください。不明な場合は「不明」と記載）
